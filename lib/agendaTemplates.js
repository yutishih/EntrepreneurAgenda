...
// 4 CHINA-only fields: voteCounter/wordOfTheDay/inductionHost/quizHost).
...
const CHINA_SCHEDULE = [
  ...
  { time: '7:22', duration: '10', program: 'Induction Ceremony', fields: ['inductionHost'] },
  ...
  { time: '8:58', duration: "3-5", program: 'General Evaluation', fields: ['generalEvaluator'] },
...
`;
  ...
  china: {
    ...
    render(data, club, ctx) {
      ...
      const tbody = CHINA_SCHEDULE.map((row) => `
...
// ENTREPRENEUR — the Entrepreneur Toastmasters Club's own dedicated template.
// A straight clone of `standard` (same layout, same render fn, same brand
// fallback defaults) under its own key/label, now that `standard` itself is
// no longer implicitly "Entrepreneur's template" — new clubs should pick the
// neutral `compact` template instead. `standard` is kept as-is (unchanged)
// purely so any existing club record with template_key='standard' keeps
// rendering exactly as before; new/edited clubs should use 'entrepreneur'.
AGENDA_TEMPLATES.entrepreneur = {
  ...AGENDA_TEMPLATES.standard,
  key: 'entrepreneur',
  label: '企業家版',
};
